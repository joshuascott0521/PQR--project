    ...
    const cargarMasPqrs = async (estado: string) => {
        ...
        try {
            ...
            setPqrsFiltrados((prev) => {
                ...
                const unicos = Array.from(new Map(combinados.map((p) => [p.id, p])).values());
                return unicos;
            });
            ...
            >
                {estadoSeleccionado && (
                    <p className="text-gray-700 font-semibold mb-2">
                        PQRs en estado: <span className="uppercase">{estadoSeleccionado}</span>
                    </p>
                )}
                ...
                {loadingPqrs && pqrsFiltrados.length > 0 && (
                    <p className="text-center text-gray-500 mt-4">Cargando más datos...</p>
                )}

                {!loadingPqrs && !hasMorePqrs && pqrsFiltrados.length > 0 && (
                    <p className="text-center text-gray-400 mt-4">No hay más resultados</p>
                )}
            </div>
